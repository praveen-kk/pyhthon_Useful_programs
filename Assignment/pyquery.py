    ...
    for i, key in enumerate(path_splits):
        ...
        if '[' in key and ']' in key and i != len(path_splits) - 1:
            ...
            if i == len(path_splits) - 1:
                ...
                if debug:
                    print('getting the whole array')
                if isinstance(doc, list):
                    if debug:
                        print('is type of array')
                else:
                    if debug:
                        print('is not type of array, constructing it manually')
                    doc = [doc]
                idx = -1
                item_arr = []
            ...
            if isinstance(doc, list):
                ...
                if debug:
                    print('getting object normaly')
                doc = doc.get(key)

            if i == len(path_splits) - 1:
                if debug:
                    print('it is the last component')
                if isinstance(doc, list):
                    if debug:
                        print('it is a list, generate a @Val array')
                    try:
...
print(get_dict_value("pythonProject/Assignment/testjs.json", "a/b/c", leaf=None))